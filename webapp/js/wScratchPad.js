		...
		else if(typeof option == 'string')
		{
			...
			var elements = this.each(function()
			{
				...
				if(data)
				{
					...
					else if(option === 'reset2') { data.reset2(); }
	...
	ScratchPad.prototype = 
	{
		...
		drawImage: function(imagePath)
		{
			...
			var settings = this.settings;
  			img.onload = function(){
  				$this.ctx.drawImage(img, 0, 0, img.width,img.height,0,0,settings.width,settings.height);
  				$this.setBgImage();
  			}
			img.src = imagePath;
		},
		...
		{
			e.pageXX = e.pageX = Math.floor(e.pageX - $this.canvas_offset.left);
			e.pageYY = e.pageY = Math.floor(e.pageY - $this.canvas_offset.top);	
			if (event == 'Down') {
				$this.lastX = e.pageXX;
				$this.lastDir = null;
			} else if (event == 'Move') {
				if (Math.abs(e.pageXX - $this.lastX) > 20) {
					if (e.pageXX - $this.lastX > 0) {
						$this.lastDir = "right";
					} else {
						$this.lastDir = "left";
					}
		...
		scratchDown: function(e, $this)
		{
			...
			$this.ctx.beginPath();
			$this.ctx.arc(e.pageXX, e.pageYY, $this.settings.size/2, 0, Math.PI*2, true);
			$this.ctx.closePath();
			$this.ctx.fill();
			
			//start the path for a drag
			$this.ctx.beginPath();
			$this.ctx.moveTo(e.pageXX, e.pageYY);
			$this.$canvas.css('margin-right', $this.$canvas.css('margin-right') == "0px" ? "1px" : "0px");
		},
		scratchMove: function(e, $this)
		{
			$this.ctx.lineTo(e.pageXX, e.pageYY);
			$this.ctx.stroke();
			$this.$canvas.css('margin-right', $this.$canvas.css('margin-right') == "0px" ? "1px" : "0px");
		},
